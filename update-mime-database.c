...
static void load_source_file(const char *filename)
{
	...
	{
		g_error("* Root element <%s> is not <mime-info>\n"
			"*   (in '%s')\n", root->name, filename);
		goto out;
	}
...
static void write_out_glob(gpointer key, gpointer value, gpointer data)
{
	...
	for ( ; list; list = list->next) {
		type = (Type *)list->data;
		if (strchr(pattern, '\n'))
			g_error("* Glob patterns can't contain literal newlines "
				"(%s in type %s/%s)\n", pattern,
...
static void check_in_path_xdg_data(const char *mime_path)
{
	...
	{
		g_error("Can't stat '%s' directory: %s",
  ...
  if (is_simple_glob (glob))
    {
      ...
      if (suffix == NULL)
	{
	  g_error ("Glob '%s' is not valid UTF-8\n", glob);
	  return;
	}
